...
def parse_args():
    ...
    parser.add_argument('--dataseed', default=6142, type=int,
                        help='')    # 2981, 6142, 1187 random seed
    ...
    # dataset
    parser.add_argument('--dataset', default='glas', help='dataset name')
    ...
    # optimizer
    parser.add_argument('--optimizer', default='HTJU',
                        choices=['Adam', 'SGD', 'NdaTJU', 'tju', 'AdamW', 'HTJU'],
                        help='loss: ' +
                        ' | '.join(['Adam', 'SGD', 'NdaTJU', 'tju', 'AdamW', 'HTJU']) +
                        ' (default: HTJU)')
    ...
    # scheduler
    parser.add_argument('--scheduler', default='MyCosineAnnealingLR',
                        choices=['CosineAnnealingLR', 'ReduceLROnPlateau', 'MultiStepLR', 'ConstantLR', "MyCosineAnnealingLR"])
...
def main():
    ...
    elif config['optimizer'] == 'NdaTJU':
        optimizer = NdaTJU(param_groups, lr=5e-3, betas=(0.9, 0.999), eps=1e-6,
                          rebound='constant', warmup=50, weight_decay_type='AdamW')

    elif config['optimizer'] == 'tju':
        optimizer = TJU(param_groups, lr=config['lr'], beta=0.9, eps=1e-6,
                         rebound='constant', warmup=50, init_lr=1e-7)

    elif config['optimizer'] == 'HTJU':
        optimizer = HTJU(param_groups, t_lr=1e-3, h_lr=1e-3,
                         t_weight_decay=5e-4, h_weight_decay=1e-4,
                         t_eps=1e-6, h_eps=1e-8, H_optim='AdamW',
                         weight_decay_type='AdamW', total_epoch=config['epochs'],
                         epoch_now=0, hessian_scale=0.05)
    ...
    elif config['scheduler'] == 'MyCosineAnnealingLR':
        scheduler = CosineAnnealingLRDual(optimizer, T_max=config['epochs'],
                                          t_lr_min=config['min_lr'],
                                          h_lr_min=config['min_lr'])
    else:
        raise NotImplementedError
    ...
    print(f"now data is {dataset_name}")
    ...
    # Initialize epoch tracking for HTJU s-weight schedule
    if hasattr(optimizer, 'epoch_now'):
        optimizer.total_epoch = config['epochs']
        optimizer.epoch_now = 0
    ...
    for epoch in range(config['epochs']):
        ...
        # HTJU: advance epoch counter for s-weight schedule
        if hasattr(optimizer, 'epoch_now'):
            optimizer.epoch_now += 1
        # Show dual LRs when using CosineAnnealingLRDual
        if hasattr(scheduler, 'get_t_lr'):
            print(f"Current t_LR: {scheduler.get_t_lr()[0]}")
            print(f"Current H_LR: {scheduler.get_h_lr()[0]}")
